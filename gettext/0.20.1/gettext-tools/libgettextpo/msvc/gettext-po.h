...
#ifdef _MSC_VER
# if IN_LIBGETTEXTPO
#  define LIBGETTEXTPO_EXPORTED __declspec(dllexport)
# else
#  define LIBGETTEXTPO_EXPORTED __declspec(dllexport)
# endif
#else
# define LIBGETTEXTPO_EXPORTED
#endif
...
extern LIBGETTEXTPO_EXPORTED int libgettextpo_version;
...
/* Return the i-th file position for a message, or NULL if i is out of
   range.  */
extern LIBGETTEXTPO_EXPORTED 
po_filepos_t po_message_filepos (po_message_t message, int i);
...
extern LIBGETTEXTPO_EXPORTED 
void po_message_set_format (po_message_t message, const char *format_type, /*bool*/int value);
...
extern LIBGETTEXTPO_EXPORTED 
const char * po_filepos_file (po_filepos_t filepos);

/* Return the line number where the string starts, or (size_t)(-1) if no line
   number is available.  */
extern LIBGETTEXTPO_EXPORTED 
size_t po_filepos_start_line (po_filepos_t filepos);
...
extern LIBGETTEXTPO_EXPORTED 
const char * po_format_pretty_name (const char *format_type);
